...
const usdAtom = atom<number>({ key: 'usd', default: 1 });
...
const eurSelector = selector<number>({
  ...
  get: ({ get }) => {
    ...
    return (
      (get(commissionEnabledAtom)
        ? removeCommission(usd, get(commissionAtom))
        : usd) * eurRate
    );
  },
  ...
  set: ({ get, set }, newValue) => {
    // @ts-ignore
    const newUsd = newValue / eurRate;
    set(
      usdAtom,
      get(commissionEnabledAtom)
        ? addCommission(newUsd, get(commissionAtom))
        : newUsd
    );
  },
});
